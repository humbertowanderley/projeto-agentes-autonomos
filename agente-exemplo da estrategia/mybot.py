		...
		if self.proxy_built:
			for warpgate in self.units(WARPGATE).ready:
				...
				if AbilityId.WARPGATETRAIN_DARKTEMPLAR in abilities:
					...
					placement = await self.find_placement(AbilityId.WARPGATETRAIN_STALKER, self.proxy1.position.to2, placement_step=5)
			...
			if self.units(DARKTEMPLAR).amount > 0 and self.units(STALKER).amount > 8:
				for templar in self.units(DARKTEMPLAR).idle:
